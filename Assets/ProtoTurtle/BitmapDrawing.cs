...
namespace ProtoTurtle.BitmapDrawing
{
    ...
    public static class BitmapDrawingExtensions
    {
        ...
        public static void FloodClear(this Texture2D texture, int startX, int startY, Color newColor)
        {
            Point start = new Point(startX, TransformToLeftTop_y(startY, texture.height));
            ...
            Color originalColor = texture.GetPixel(start.X, start.Y);
            //Color originalColor = new Color(0, 0, 0, 1);
            int width = texture.width;
            int height = texture.height;
            ...
            while (openNodes.Count > 0)
            {
                ...
                if (x > 0)
                {
                    var distance = Mathf.Abs(copyBmp[x - 1, y].r - originalColor.r) + Mathf.Abs(copyBmp[x - 1, y].g - originalColor.g) + Mathf.Abs(copyBmp[x - 1, y].b - originalColor.b);
                    if (distance <= 0.7)
                    {
                        copyBmp[x - 1, y] = newColor;
                        openNodes.Enqueue(new Point(x - 1, y));
                ...
                if (y > 0)
                {
                    var distance = Mathf.Abs(copyBmp[x , y - 1].r - originalColor.r) + Mathf.Abs(copyBmp[x, y - 1].g - originalColor.g) + Mathf.Abs(copyBmp[x , y - 1].b - originalColor.b);
                    if (distance <= 0.7)
                    {
                        copyBmp[x, y - 1] = newColor;
                        openNodes.Enqueue(new Point(x, y - 1));
